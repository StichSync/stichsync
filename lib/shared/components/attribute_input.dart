...
class AttribInput extends StatefulWidget {
  ...
  const AttribInput(
      {super.key, required this.addAttribute, required this.removeAttribute});
...
class AttribInputState extends State<AttribInput> {
  ...
  void pressed(AttribInputType type, bool up) {
    setState(() {
      List<String> lenghtUnits =
          ShortLengthUnit.values.map((e) => e.name).toList();
      List<String> weightUnits =
          ShortWeightUnit.values.map((e) => e.name).toList();
      List<String> types = AttributeType.values.map((e) => e.name).toList();
      List<String> lenghtTypes =
          LengthAttributeType.values.map((e) => e.name).toList();
      switch (type) {
        case AttribInputType.type:
          ...
          if (up) {
            if (types.indexOf(name) + 1 < types.length) {
              assetDir =
                  "assets/${types[types.indexOf(name) + 1].toString()}.png";
            } else {
                ...
                SizedBox(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                            flex: 3, child: Image(image: AssetImage(assetDir))),
                        Expanded(
                          flex: 2,
                          child: Column(
